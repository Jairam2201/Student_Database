...
def details():
    ...
    if row==None:
        ...
        frame=Frame(update,bg="CadetBlue1")
        ...
        def change():
            ...
            query="use student_dtbase"
            mycursor.execute(query)
            query="update student_details set fname=%s,mname=%s,lname=%s,inter_clgname=%s,inter_marks=%s,mobile_no=%s,gender=%s,email_id=%s,place=%s,degree=%s,branch=%s,state=%s where id=%s"
            mycursor.execute(query,(firstname.get(),mname.get(),lname.get(),inter_clgname.get(),inter_marks.get(),phno.get(),sel_gender.get(),email.get(),place.get(),sel_degree.get(),sel_branch.get(),sel_states.get(),rollno.get()))
            ...
            con.commit()
            con.close()
        button=Button(update,text="UPDATE",font=("Times New Roman",18,"bold"),cursor="hand2",bg="DeepSkyBlue2",width=22,command=change)
...
update=Tk()
update.title("CHANGE DETAILS")
width= update.winfo_screenwidth()
height= update.winfo_screenheight()
update.geometry("%dx%d" % (width, height))
back_icon=PhotoImage(file="back_icon.png")
back=Button(update,image=back_icon,bd=0,bg="white",fg="orange red",text="Back",width=70,compound=LEFT,cursor="hand2",font=("Times New Roman",15,"bold"),command=exit)
back.place(x=50,y=20)
label=Label(update,text="Student Rollno : ",font=("Times New Roman",20,"bold"))
label.place(x=500,y=80)
frame=Frame(update,highlightbackground="gray55",highlightthickness=1,highlightcolor="orange red")
frame.place(x=700,y=84)
rollno=Entry(frame,font=("Times New Roman",18,"bold"),bd=0,bg="gray87")
rollno.grid()
submit=Button(update,text="Submit",bg="blue",fg="white",activebackground="blue4",activeforeground="white",cursor="hand2",font=("Times New Roman",15,"bold"),command=details)
