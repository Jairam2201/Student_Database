...
def details():
    ...
    if row==None:
        ...
        frame=Frame(delete,bg="CadetBlue1")
        ...
        def dlete():
            ...
            con.commit()
            con.close()
        button=Button(delete,text="DELETE",font=("Times New Roman",18,"bold"),bg="DeepSkyBlue2",cursor="hand2",width=22,command=dlete)
        button.place(x=550,y=650)


delete=Tk()
delete.title("DELETE")
width= delete.winfo_screenwidth()
height= delete.winfo_screenheight()
delete.geometry("%dx%d" % (width, height))
back_icon=PhotoImage(file="back_icon.png")
back=Button(delete,image=back_icon,text="Back",bd=0,bg="white",fg="orange red",compound=LEFT,width=70,font=("Times New Roman",15,"bold"),cursor="hand2",command=exit)
back.place(x=50,y=20)
label=Label(delete,text="Student Rollno : ",font=("Times New Roman",20,"bold"))
label.place(x=500,y=80)
frame=Frame(delete,highlightbackground="gray55",highlightthickness=1,highlightcolor="orange red")
frame.place(x=700,y=84)
rollno=Entry(frame,font=("Times New Roman",18,"bold"),bd=0,bg="gray87")
rollno.grid()
submit=Button(delete,text="Submit",bg="blue",fg="white",activebackground="blue4",activeforeground="white",cursor="hand2",font=("Times New Roman",15,"bold"),command=details)
